...
def validate_passwords_1(filename):
    valid_passwords = 0

    for line in lines(filename):
        line_parts = line.split()
        password_text = line_parts[2]
        required_char = line_parts[1][0]
        char_repeats_min, char_repeats_max = [int(part) for part in line_parts[0].split('-')]

        if char_repeats_min <= password_text.count(required_char) <= char_repeats_max:
            valid_passwords += 1

    print(f"Part 1 Solution: {valid_passwords}")


def validate_passwords_2(filename):
    valid_passwords = 0
    ...
    for line in lines(filename):
        ...
        password_text = line_parts[2]
        required_char = line_parts[1][0]
        char_position_1, char_position_2 = [int(part) - 1 for part in line_parts[0].split('-')]

        chars_under_test = {password_text[char_position_1], password_text[char_position_2]}

        if len(chars_under_test) == 2 and required_char in chars_under_test:
            valid_passwords += 1
...
if __name__ == "__main__":
    filename = 'input.txt'
    validate_passwords_1(filename)
    validate_passwords_2(filename)
