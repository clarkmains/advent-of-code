...
def count_trees(rows, move_x, move_y):
    trees = 0
    x_pos = 0
    y_pos = move_y
    ...
    while y_pos < len(rows):
        ...
        x_pos = (x_pos + move_x) % len(rows[y_pos])
...
if __name__ == "__main__":
    ...
    part_2_totals = [count_trees(rows, slope[0], slope[1]) for slope in slopes]
    part_2_solution = prod(part_2_totals)
