...
def find_seat(moves, move_higher, move_lower, max):
    min = 0
    ...
    for move in moves:
        ...
        if move == move_higher:
            max = middle
        elif move == move_lower:
            min = middle + 1

    return min


def find_remaining_seat(seats):
    seats_set = set(seats)
    all_seats_set = set(list(range(min(seats), max(seats))))
    return all_seats_set.symmetric_difference(seats_set).pop()
...
if __name__ == "__main__":
    ...
    for line in lines(filename):
        col = find_seat(line[:7], 'F', 'B', ROWS)
        row = find_seat(line[7:], 'L', 'R', COLS)
        seat_id = col * 8 + row
        max_seat = max(max_seat, seat_id)
        seats.append(seat_id)

    print(f"Part 1 Solution: {max_seat}")
    print(f"Part 2 Solution: {find_remaining_seat(seats)}")
