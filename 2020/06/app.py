    ...
    with open(filename) as file:
        for line in file:
            if line == NEW_LINE:
                ...
                groups[key].append(line.rstrip())

    return groups


def sum_all_answers(groups):
    """For each group, count the number of questions to which anyone answered "yes".
...
def sum_consensus_answers(groups):
    """For each group, count the number of questions to which everyone answered "yes".
    Return the sum of those counts. """
    total = 0
    ...
    for group in groups.values():
        ...
        for letter in group_answers:
            if group_answers[letter] == group_size:
                total += 1
...
if __name__ == "__main__":
    ...
    print(f"Part 1: {sum_all_answers(groups)}")
    print(f"Part 2: {sum_consensus_answers(groups)}")
