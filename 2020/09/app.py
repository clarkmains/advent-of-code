...
use of several paperclips. Upon connection, the port outputs a series of numbers
(your puzzle input).
...
25 numbers ago) was 20. Now, for the next number to be valid, there needs to be some pair of numbers among 1-19, 21-25, or 45 that add up to it:
...
def valid_number(number, group):
    return number in {a + b for a, b in itertools.combinations(group, 2)}
...
if __name__ == "__main__":
    ...
    contiguous_numbers_sum = find_contiguous_numbers(invalid_number, numbers)

    print(f"Part 1 Solution: {invalid_number}")
    print(f"Part 2 Solution: {contiguous_numbers_sum}")
